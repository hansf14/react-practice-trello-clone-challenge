...
export const defaultCategoryTaskItems: Category[] = [
  {
...
export const defaultItems: ParentItem[] = defaultCategoryTaskItems.map(
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ({ text, id, items: _items = [] }) => {
    const items = _items.map(({ text, ...otherProps }) => ({
      ...otherProps,
      content: text,
    }));
    return {
